...
exports.genReport = function(startDate, cutoffDate, callback) {
  var stmt = "SELECT DATE_FORMAT(r1.Date, '%d %b %y') AS 'StartDate', DATE_FORMAT(r2.Date, '%d %b %y') AS 'CutOffDate', r1.Unit, r2.Reading - r1.Reading AS 'Reading (kWh)' FROM daily_last_reading r1, daily_last_reading r2 WHERE r1.Unit = r2.Unit AND (r1.Unit, r1.Date) IN (SELECT Unit, MIN(Date) FROM daily_last_reading WHERE Date BETWEEN ? AND ? GROUP BY Unit) AND r2.Date = ? ORDER BY r1.Unit;";
